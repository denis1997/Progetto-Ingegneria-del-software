...
Public Class Form25
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        Try
            connection.Open()
            Dim query As String
            query = "select idedificio, idposizione from piano where id=@id"
            command = New MySqlCommand(query, connection)
            ...
            End If
            READER.Close()
            query = "select count(id) from piano where idedificio = @id"
            command = New MySqlCommand(query, connection)
            ...
            For i As Integer = p To b
                query = "UPDATE piano SET idposizione=@id1 where idposizione=@id2 and idedificio=@id3"
                command = New MySqlCommand(query, connection)
            ...
            query1 = "delete from piano where(id) = ('" & TextBox3.Text & "') "
            command = New MySqlCommand(query1, connection)
            READER = command.ExecuteReader

            MessageBox.Show("Piano rimosso")
