...
Public Class Form24
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        Try
            connection.Open()
            Dim query As String
            query = "select idzona, idposizione from edificio where id=@id"
            command = New MySqlCommand(query, connection)
            ...
            End If
            READER.Close()
            query = "select count(id) from edificio where idzona = @id"
            command = New MySqlCommand(query, connection)
            ...
            For i As Integer = p To b
                query = "UPDATE edificio SET idposizione=@id1 where idposizione=@id2 and idzona=@id3"
                command = New MySqlCommand(query, connection)
            ...
            query1 = "delete from edificio where(id) = ('" & TextBox3.Text & "') "
            command = New MySqlCommand(query1, connection)
            READER = command.ExecuteReader

            MessageBox.Show("Edificio rimosso")
