...
Public Class Form16
    ...
    Private Sub Form13_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        Try
            connection.Open()
            Dim query As String
            query = "select id, nome from areapiano"
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        Try
            connection.Open()
            Dim query As String
            query = "select idpiano, idposizione from areapiano where id=@id"
            command = New MySqlCommand(query, connection)
            ...
            End If
            READER.Close()
            query = "select count(id) from areapiano where idpiano = @id"
            command = New MySqlCommand(query, connection)
            ...
            For i As Integer = p To b
                query = "UPDATE areapiano SET idposizione=@id1 where idposizione=@id2 and idpiano=@id3"
            ...
            query1 = "delete from areapiano where(id) = ('" & TextBox3.Text & "') "
            command = New MySqlCommand(query1, connection)
            READER = command.ExecuteReader

            MessageBox.Show("AreaPiano rimossa")
